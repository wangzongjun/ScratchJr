...
export default class UI {
    ...
    static leftPanel (div) {
        ...
        onTouchStartBind(flip,function (evt) {
    ...
    static addProjectInfo () {
        ...
        onTouchStartBind(shareEmail,function (e) {
        ...
        if (!isAndroid) {
            ...
            onTouchStartBind(shareAirdrop,function (e) {
        ...
        onTouchStartBind(saveCloudButton,function (e) {
            ...
            Project.prepareToSave(ScratchJr.currentProject, function () {
                IO.zipProject(ScratchJr.currentProject, function (contents) {
                    window.JrConfig.onSaveCloud(contents, IO.zipFileName)
                    // uploadFile(contents,IO.zipFileName + ".sjr", (res)=>{
                    //     res=JSON.parse(res);
                    //     if(res.success){
                    //         submitWork(IO.zipFileName,)
                    //     }
                    // })
        ...
        // uploadFile = function(contents, filename, cb){
        ...
        onTouchEndBind(info,UI.showInfoBox);
        onTouchStartBind(okclicky,UI.hideInfoBox);
        onTouchStartBind(okclicky,function (evt) {
    ...
    static parentalGate (evt, callback) {
        ...
        onTouchStartBind(pgCloseButton,function () {
        ...
        onTouchStartBind(pgChoiceA,function () {
            parentalGateClose(theProblem[4] == 0);
        });
        onTouchStartBind(pgChoiceB,function () {
            parentalGateClose(theProblem[4] == 1);
        });
        onTouchStartBind(pgChoiceC,function () {
    ...
    static showInfoBox (e) {
        ...
        setTimeout(function () {
            onTouchEndBind(okclicky,UI.hideInfoBox);
    ...
    static layoutLibrary (sl) {
        ...
        onTouchStartBind(div,UI.spriteThumbsActions);
        ...
        if (ScratchJr.isEditable()) {
            ...
            onTouchStartBind(ns,UI.addSprite);
        }
    }
    ...
    static layoutToolbar (div) {
        ...
        onTouchStartBind(addt,UI.addText);
        var changebkg = newHTML('div', 'changeBkg', tb);
        onTouchStartBind(changebkg,UI.addBackground);
    }
    ...
    static createFormForText (p) {
        ...
        onTouchStartBind(clicky,UI.openFontSizeMenu);
        var col = newHTML('div', 'changecolorText off', ta);
        col.setAttribute('id', 'fontcolorbutton');

        onTouchStartBind(col,UI.topLevelColor);
    ...
    static createColorMenu (div) {
        ...
        for (var i = 0; i < swatchlist.length; i++) {
            ...
            onTouchStartBind(colour,UI.setTextColor);
        }
    ...
    static createTextSizeMenu (div) {
        ...
        for (var i = 0; i < sizes.length; i++) {
            ...
            sf.textContent = 'A';
            onTouchStartBind(textuisize,UI.setTextSize);
        }
