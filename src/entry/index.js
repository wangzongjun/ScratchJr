...
import {gn, getUrlVars, isAndroid, isiOS, onTouchStartBind, onTouchMoveBind, onTouchEndBind} from '../utils/lib';
import iOS from '../tablet/OS';
...
export function indexMain () {
    onTouchEndBind(gn('gettings'),indexGettingstarted);
    onTouchEndBind(gn('startcode'),indexGohome);
    ScratchAudio.init();
    ...
    if (window.Settings.edition == 'PBS') {
        ...
        onTouchEndBind(gn('startButton'),indexGohome);
        onTouchEndBind(gn('pbschars'),indexGohome);

        onTouchStartBind(gn('topbar-moreapps'),indexMoreApps);
        onTouchStartBind(gn('topbar-settings'),indexGoSettings);
        onTouchStartBind(gn('topbar-info'),indexInfo);
    } else {
        onTouchStartBind(gn('gear'),indexGoSettings);
    }
...
function indexAskPlace () {
    ...
    onTouchEndBind(gn('usageSchool'),indexSetUsage);
    onTouchEndBind(gn('usageHome'),indexSetUsage);
    onTouchEndBind(gn('usageOther'),indexSetUsage);
    onTouchEndBind(gn('usageNoanswer'),indexSetUsage);
}
