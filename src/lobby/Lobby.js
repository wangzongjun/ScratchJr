...
import {libInit, getUrlVars, gn, isAndroid, newHTML, onTouchStartBind, onTouchMoveBind, onTouchEndBind} from '../utils/lib';
...
export default class Lobby {
    ...
    static appinit (v) {
        ...
        onTouchStartBind(gn('hometab'),function () {
            if (gn('hometab').className != 'home on') {
                Lobby.setPage('home');
            }
        });
        onTouchStartBind(gn('helptab'),function () {
            if (gn('helptab').className != 'help on') {
                Lobby.setPage('help');
            }
        });
        onTouchStartBind(gn('booktab'),function () {
            if (gn('booktab').className != 'book on') {
                Lobby.setPage('book');
            }
        });
        onTouchStartBind(gn('geartab'),function () {
            if (gn('geartab').className != 'gear on') {
                Lobby.setPage('gear');
            }
        });
        onTouchStartBind(gn('abouttab'),function () {
            if (gn('abouttab').className != 'tab on') {
                Lobby.setSubMenu('about');
            }
        });
        onTouchStartBind(gn('interfacetab'),function () {
            if (gn('interfacetab').className != 'tab on') {
                Lobby.setSubMenu('interface');
            }
        });
        onTouchStartBind(gn('painttab'),function () {
            if (gn('painttab').className != 'tab on') {
                Lobby.setSubMenu('paint');
            }
        });
        onTouchStartBind(gn('blockstab'),function () {
    ...
    static loadSettings (p) {
        ...
        for (var l in window.Settings.supportedLocales) {
            ...
            onTouchStartBind(languageButton,function (e) {
                ScratchAudio.sndFX('tap.wav');
